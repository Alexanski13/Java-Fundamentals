...
public class PasswordValidator {
    public static void main(String[] args) {
        ...
        if (passLengthValidation(password) && isConstitOnlyLettersAndDigits(password) && hasAtLeast2Digits(password)) {
            System.out.println("Password is valid");
        }
        if (!passLengthValidation(password)) {
            System.out.println("Password must be between 6 and 10 characters");
        }
        if (!isConstitOnlyLettersAndDigits(password)) {
    ...
    private static boolean isConstitOnlyLettersAndDigits(String pass) {
        for (int i = 0; i < pass.length(); i++) {
